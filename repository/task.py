...
class TaskRepository:
    ...
    # это уже хз работает или нет
    async def get_user_task(self, task_id: int, user_id: int) -> Tasks | None:
        query = (
            select(Tasks, UserProfile)
            # .join(Categories, Categories.id == Tasks.category_id)          # это неправильно
            .join(UserProfile, UserProfile.id == Tasks.user_id)
